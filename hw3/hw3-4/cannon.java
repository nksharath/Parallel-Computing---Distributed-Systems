...
public class cannon
{
    ...
    {   
        ...
        //Comm world = Comm.world();
         if(args.length<3){
            System.out.println("Example: java -Dpj.np=9 cannon N Seed filename");
            System.exit(1);
        }
        world=Comm.world();
        rank = world.rank();
        ...
        //Pefrom row shift for cannons algorithm
        cannonshiftrow();
        //Perform columnshift for cannons algorithm
        cannonshiftcol();
        
        //Perform multiplication 
        cannonperformMultiplication();
   ...
   static void ParticipatingRow(int a)throws Exception
   {
    ...
    /*
     * cannonperfomrMultiplication  performs matrix multiplication 
     * 
     * 
     */
   static void cannonperformMultiplication()
   {
       for(int i=0;i<(int)N/(int)Math.sqrt(size);i++)
           for(int j=0;j<(int)N/(int)Math.sqrt(size);j++)
               for(int k=0;k<N;k++)
  ...
  static void  writeMatrix(String filename)throws Exception
   ...
   /*
     * cannonshiftrow performs cyclic row shift of matrix a
     * 
     */
  static void cannonshiftrow()
  {
      for(int i=0;i<((int)N/(int)Math.sqrt(size));i++)
        {
            for(int j=0;j<((int)N/(int)Math.sqrt(size));j++)
       ...
       for(int i=0;i<((int)N/(int)Math.sqrt(size));i++)
        {
            for(int j=0;j<((int)N/(int)Math.sqrt(size));j++)
            {
               for(int k=0;k<i;k++)
               {
                   int temp=matrixb[i][k];
                   matrixb[i][k]=matrixb[i][k+1];
                   matrixb[i][k]=temp;
               }
            }
        }
